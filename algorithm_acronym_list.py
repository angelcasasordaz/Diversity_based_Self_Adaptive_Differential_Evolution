...
CUSTOM_OPTIMIZER_ALIASES = {
    ...
    "DE_DIVERSITYSELECTION": "DE-DiversitySelection",
    "DE-MAHALANOBIS": "DE-Mahalanobis",
    "DE_MAHALANOBIS": "DE-Mahalanobis",
}
...
def _available_optimizer_names() -> dict[str, str]:
    return {optimizer_name.casefold(): optimizer_name for optimizer_name in _available_optimizers()}
...
def optimizer_class(name: str):
    """Return the installed MEALPY optimizer class for a resolved/non-custom name."""
    resolved_name = resolve_optimizer_name(name)
    if resolved_name in CUSTOM_OPTIMIZERS:
        raise ValueError(f"'{name}' is a custom optimizer and must be constructed locally")
